...
<script>
import { listMathTf, getMathTf, delMathTf, addMathTf, updateMathTf } from "@/api/math/mathTf";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getMathTf(questionId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.questionId != null) {
            updateMathTf(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addMathTf(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('math/mathTf/export', {
        ...this.queryParams
      }, `mathTf_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
