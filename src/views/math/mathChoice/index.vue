    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['math:mathChoice:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['math:mathChoice:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['math:mathChoice:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['math:mathChoice:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listMathChoice, getMathChoice, delMathChoice, addMathChoice, updateMathChoice } from "@/api/math/mathChoice";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getMathChoice(questionId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.questionId != null) {
            updateMathChoice(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addMathChoice(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除选择题编号为"' + questionIds + '"的数据项？').then(function() {
        return delMathChoice(questionIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('math/mathChoice/export', {
        ...this.queryParams
      }, `mathChoice_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
