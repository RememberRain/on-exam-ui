    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['english:englishChoice:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['english:englishChoice:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['english:englishChoice:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['english:englishChoice:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listEnglishChoice, getEnglishChoice, delEnglishChoice, addEnglishChoice, updateEnglishChoice } from "@/api/english/englishChoice";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getEnglishChoice(questionId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.questionId != null) {
            updateEnglishChoice(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addEnglishChoice(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除选择题编号为"' + questionIds + '"的数据项？').then(function() {
        return delEnglishChoice(questionIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('english/englishChoice/export', {
        ...this.queryParams
      }, `englishChoice_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
